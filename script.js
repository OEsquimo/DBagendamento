...
function renderServiceForms() {
    ...
    servicosSelecionados.forEach(service => {
        const slideElement = document.createElement('li'); // Usamos <li> para listas semânticas dentro do <ul>
        slideElement.className = 'splide__slide service-form-group'; // Classes essenciais para Splide
    ...
    // Adicionar os event listeners para os campos após eles serem renderizados
    document.querySelectorAll('.additional-field-select, .additional-field-input, .additional-field-select-no-price, .additional-field-quantidade, .additional-field-textarea').forEach(field => {
        field.addEventListener('change', updatePrice);
        field.addEventListener('input', updatePrice); // Para campos numéricos e de texto
    });
...
function initializeSplideCarousel() {
    // Verifica se já existe uma instância do Splide para evitar duplicatas
    const splideInstance = document.getElementById('servicosCarousel')?.splide;
    if (splideInstance) {
        splideInstance.destroy(); // Destroi a instância anterior se ela existir
    }

    const splide = new Splide('#servicosCarousel', {
        type: 'slide',
        perPage: 1,
        gap: '1rem',
        pagination: false,
    ...
    // Adiciona a instância do Splide ao elemento do container para referência
    document.getElementById('servicosCarousel').splide = splide;

    splide.mount(); // Monta o carrossel
}
